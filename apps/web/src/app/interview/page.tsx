...
const STAGES = [
  ...
  },
  {
    ...
    description: 'CS fundamentals, problem solving',
  },
  {
    id: 'DSA_LIVE',
    label: 'DSA Live',
    description: 'Coding problem with live feedback',
  },
  {
    id: 'SYSTEM_DESIGN',
    label: 'System Design',
  ...
  },
  {
    id: 'OFFER_DEBRIEF',
    label: 'Offer Debrief',
    description: 'Negotiation and questions to ask',
  },
];
...
export default function InterviewPage() {
  ...
  const startStage = async (stageId: string) => {
    ...
    try {
      const { data } = await interviewApi.message({
        user_id: 'test-user',
        stage: stageId,
        history: [],
        message:
          'Start the interview. Introduce yourself briefly and ask your first question.',
      });
      ...
      setMessages([
        {
          role: 'assistant',
          content:
            'Could not start interview. Check that the AI engine is running.',
        },
      ]);
    } finally {
      setLoading(false);
  ...
  const getFeedback = async () => {
    ...
    try {
      const { data } = await interviewApi.feedback({
        stage,
        transcript: messages,
      });
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">
            Choose a stage to begin your session.
          </p>
        </div>
          ...
          {STAGES.map((s, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <ChevronRight
                  size={16}
                  className="text-white/20 group-hover:text-[#1B6CF2] transition-colors"
                />
              </div>
            </button>
          ))}
      ...
      {feedback && (
        <div className="bg-[#F0A500]/5 border border-[#F0A500]/20 rounded-xl p-5 shrink-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[#F0A500] text-sm font-medium">
              Session Feedback
            </span>
          </div>
